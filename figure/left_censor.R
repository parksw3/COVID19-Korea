...
covid1 <- read_xlsx("COVID19-Korea-2020-02-31.xlsx", sheet=1, na="NA")

covid1_subset <- covid1 %>%
  filter((!is.na(date_discharged) | !is.na(date_onset))) %>%
  select(
...
covid1_unknown <- covid1 %>%
  ...
  filter((symptoms!="none before confirmation" | is.na(symptoms)), is.na(date_onset)) %>%
  mutate(
...
g1 <- ggplot(covid1_subset_gather) +
  geom_segment(data=covid1_subset, aes(x=date_onset, xend=date_confirm, y=case, yend=case),
...
ggsave("figure_patient_timeline.png", g1, width=10, height=10)
